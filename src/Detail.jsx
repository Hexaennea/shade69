...
const GRID = {
  ...
  salary:  "36px 1fr 160px 110px 130px",
};

export default function Detail({ row, employees, onBack }) {
  const activeEmps  = employees.filter(e => !e[C.emp_disc]);
  const totalSalary = activeEmps.reduce((s, e) => s + Number(e[C.emp_salary] || 0), 0);
  const profit      = row.totalInvoice - row.totalTDS - row.totalPetty - totalSalary;
    ...
    <div className="slide-right">
      ...
      <div className="detail-section">
        <SectionHeader title="Employee Salaries" color="var(--purple)"
          count={`${activeEmps.length} active · ${rupee(totalSalary)}`} />
        <div className="table-shell">
          <div className="table-scroll">
            <div className="table-head" style={{ display: "grid", gridTemplateColumns: GRID.salary, minWidth: 660 }}>
              {[["#","var(--text3)"],["Name","var(--purple)"],["Designation","var(--purple)"],
                ["Basic + HRA","var(--blue)"],["Monthly Salary","var(--purple)"]
              ].map(([h, c], i) => <div key={i} className="th" style={{ color: c }}>{h}</div>)}
            </div>
            {activeEmps.length === 0
              ? <div style={{ padding: "22px 18px", color: "var(--text3)", fontSize: 13 }}>No active employees.</div>
              : activeEmps.map((e, i) => (
                <div key={e[C.emp_id] ?? i} className="table-row fade-up"
                  style={{ gridTemplateColumns: GRID.salary, minWidth: 660, animationDelay: `${i * .03}s` }}>
                  <div className="td" style={{ color: "var(--text3)", fontSize: 12, fontFamily: "var(--mono)" }}>{i + 1}</div>
                  <div className="td">
                    <div style={{ fontWeight: 700, fontSize: 13, color: "var(--text)", marginBottom: 2 }}>{e[C.emp_name]}</div>
                    <div style={{ fontSize: 11, color: "var(--text3)" }}>{e[C.emp_location] || ""}</div>
                  </div>
                  <div className="td" style={{ fontSize: 12, color: "var(--text2)" }}>{e[C.emp_desig] || "—"}</div>
                  <div className="td" style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--blue)" }}>
                    {rupee(Number(e[C.emp_basic] || 0) + Number(e[C.emp_hra] || 0))}
                  </div>
                  <div className="td" style={{ fontFamily: "var(--mono)", fontSize: 13, color: "var(--purple)", fontWeight: 600 }}>
                    {rupee(e[C.emp_salary])}
                  </div>
                </div>
              ))
            }
